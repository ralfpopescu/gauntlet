...
type UpdatePlayerInput = { playerIndex: PlayerIndex, player: PlayerType }
...
export const appStateSlice = createSlice({
  ...
  reducers: {
    ...
    updatePlayer: (state, action: PayloadAction<UpdatePlayerInput>) => {
        const player = getPlayerByIndex(state, action.payload.playerIndex)
        const updatedPlayer = { ...player, ...action.payload.player };
    ...
    recordEvents: (state, action: PayloadAction<Event[]>) => {
        state.events = [...state.events, ...action.payload]
    },
  },
})
